...
for index, alpha in enumerate(l_alpha):
    ...
    # input to E plot at low and high baseline states (Fig. S4A, B)
    if (alpha == 0 or alpha == 15):
        plt.figure()

        plt.plot(l_e_e, color='blue')
        ...
        if alpha == 0:
            ...
            plt.title('Low baseline state non-ISN')

            plt.yticks([-8, -4, 0, 4, 8])
            plt.ylim([-8, 8])

            plt.savefig('Fig_S3A.png')
            plt.close()
        else:
            plt.hlines(y=19.9, xmin=50000, xmax=70000, color='gray')
            
            plt.title('High baseline state non-ISN')
            plt.yticks([-20, -10, 0, 10, 20])
            plt.ylim([-20, 20])

            plt.savefig('Fig_S3B.png')
            plt.close()

# difference in input to E between baseline and top-down stimulation (Fig. S4C)
plt.figure()

s_title_t1 = 'Input to E stim-baseline non-ISN'

plt.plot(e_high - e_low, color='blue')
