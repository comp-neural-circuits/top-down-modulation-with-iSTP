    ...
    for alpha in l_alpha:
        ...
        for i in range(T):
            ...
            if 40000 < i < 45000:
                ...
                K_SV_STP = (p_pp + p_pp_prime * r_p - 1) * Jee * Jpp * Jsv - (p_pp + p_pp_prime * r_p - 1) * Jpp * Jsv -(p_ep + p_ep_prime * r_p -1) * Jsv * Jpe * Jep
                K_SV_nonSTP = Jee * Jpp * Jsv - Jsv * Jpe * Jep - Jpp * Jsv + Jee * Jsv - Jsv

                R_SV = 1/det_mat_R * K_SV * c
                R_SV_STP = 1/det_mat_R * K_SV_STP * c
                R_SV_nonSTP = 1/det_mat_R * K_SV_nonSTP * c

                l_R_SV_temp.append(R_SV)
                l_R_SV_nonSTP_temp.append(R_SV_nonSTP)
        ...
        l_R_SV_nonSTP_temp = np.array(l_R_SV_nonSTP_temp)
        ...
        l_R_SV_non_STP.append(np.mean(l_R_SV_nonSTP_temp))
        l_R_SV_STP.append(np.mean(l_R_SV_STP_temp))

    # plot change in SST activity vs R_SV for different perturbation (Fig. 3 A,B)
    plt.figure()
