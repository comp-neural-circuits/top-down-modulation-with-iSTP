...
for j, Jep in enumerate(l_Jep):
    for k, Jes in enumerate(l_Jes):
        ...
        if not unstable:
            m_response_reversal_index[j, k] = compute_response_reversal_index(
                l_change_SST
            )
        else:
...
sio.savemat(
    "m_response_reversal_index_Jep_Jes.mat",
    mdict={"m_response_reversal_index": m_response_reversal_index},
)
...
sio.savemat(
    "m_response_reversal_index_Jep_Jpe.mat",
    mdict={"m_response_reversal_index": m_response_reversal_index},
)
...
for j, Jes in enumerate(l_Jes):
    for k, Jse in enumerate(l_Jse):
        ...
        # compute response reversal index
        if not unstable:
            m_response_reversal_index_Jes_Jse[j, k] = compute_response_reversal_index(
                l_change_SST
            )
        else:
...
sio.savemat(
    "m_response_reversal_index_Jes_Jse.mat",
    mdict={"m_response_reversal_index": m_response_reversal_index_Jes_Jse},
)
...
sio.savemat(
    "m_response_reversal_index_Jse_Jpe.mat",
    mdict={"m_response_reversal_index": m_response_reversal_index},
)
...
for file_name, section in zip(l_file_name, sections):
    m_response_reversal_index = sio.loadmat(
        "m_response_reversal_index" + file_name + ".mat"
    )["m_response_reversal_index"]
    ...
    for i, j in zip(*np.where(indices_one_dead)):
        g.add_patch(
            plt.Rectangle(
                (j, i), 1, 1, facecolor="grey", edgecolor="black", linewidth=3
            )
        )
    ...
    if file_name == "_Jep_Jpe":
        ...
        ax2 = plt.twinx()
        sns.lineplot(
            data=np.arange(0, m_response_reversal_index.shape[1] + 1, 1),
            color="black",
            ax=ax2,
        )
        plt.yticks([])
