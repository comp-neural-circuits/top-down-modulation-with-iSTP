...
for k, alpha in enumerate(l_alpha):
    ...
    for i in range(T):
        if 5000 <= i < 7000:
            g_e, g_p, g_s, g_v = 4 + alpha, 4 + alpha, 3 + c, 4
        else:
            g_e, g_p, g_s, g_v = 4 + alpha, 4 + alpha, 3, 4
...
# R_SS and R_SV values vs alpha (Fig. 5A)
fig = plt.subplots(figsize=(20,10))

markers_on = [0, 30]

plt.plot(l_R_SV, '-D', markevery=markers_on, color=pal[1])
plt.plot(l_R_SS, '-D', markevery=markers_on, color=pal[2])
...
plt.legend(['$R_{SV}$', '$R_{SS}$'], loc='best')
plt.savefig('R_values.png')
plt.close()

# example activity plots SST perturbation -- paradoxical effect (Fig. 5B)
for i in [0, 30]:
    plt.figure()
    ...
    plt.hlines(y=1, xmin=30000, xmax=90000, colors='k', linestyles=[(0, (6, 6, 6, 6))])
    ...
    if i==0:
        ...
        plt.savefig('Low_baseline_network_activity_SST_perturbation.png')
        plt.close()
    if i==30:
        plt.title('High baseline network activity')
        plt.savefig('High_baseline_network_activity_SST_perturbation.png')
        plt.close()